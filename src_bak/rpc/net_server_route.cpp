...
bool RpcServer_Route_Client::Send(Controller* cntl, const RpcMeta& meta, const google::protobuf::Message& body){
...
bool RpcServer_Route::Send(Controller* cntl, const RpcMeta& meta, const google::protobuf::Message& body){
	if(cntl == NULL){ RPC_DEBUG_E("controller is null."); return false; }

	set<RouteID> route_ids;
	for(int index = 0; index < cntl->link_size(); ++index){
		RPC_DEBUG_P("send to route [%d] route length [%d] link id [%d]", GetRouteID(cntl->link_id(index)), route_length_, cntl->link_id(index));
		route_ids.insert(GetRouteID(cntl->link_id(index)));
	}

	bool ret(!route_ids.empty());
	for(auto route_id : route_ids){
		if(route_id < route_client_list_.size()){
			ret &= (route_client_list_[route_id]->Send(cntl, meta, body) == eNetSendResult_Ok);
		}
	}
	return ret;
}
